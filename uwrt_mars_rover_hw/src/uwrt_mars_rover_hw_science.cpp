...
bool UWRTRoverHWScience::init(ros::NodeHandle& /*root_nh*/, ros::NodeHandle& robot_hw_nh) {
  ...
  for (const auto& name : joint_names_) {
    ...
    hardware_interface::JointStateHandle state_handle(name, &joint_states_[name].pos,
                                                            &joint_states_[name].vel, 
                                                            &joint_states_[name].pos);
    joint_state_interface_.registerHandle(state_handle);

    // joint command interfaces
    hardware_interface::JointHandle cmd_handle(joint_state_interface_.getHandle(name), 
